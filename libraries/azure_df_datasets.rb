...
class AzureDFDataSets < AzureGenericResources
  name 'azure_df_datasets'
  desc 'Verifies settings for Azure Virtual Machines'
  example <<-EXAMPLE
    azure_df_datasets(resource_group: 'example') do
      it{ should exist }
    end
  EXAMPLE
  ...
  def initialize(opts = {})
    ...
    # providers/Microsoft.DataFactory/factories/{factoryName}/datasets?api-version=2018-06-01
    #
    # The dynamic part that has to be created for this resource:
    #   Microsoft.DataFactory/factories/{factoryName}/datasets?api-version=2018-06-01
    #
    ...
    table_schema = [
      ...
      { column: :dataset_types, field: :dataset_type },
      { column: :dataset_folder_paths, field: :dataset_folder_path },
      { column: :dataset_file_names, field: :dataset_file_name },
      { column: :dataset_file_types, field: :dataset_file_type },
      { column: :dataset_formats, field: :dataset_format },
    ]
  ...
  def populate_table
    ...
    @resources.each do |resource|
      @table << {
        ...
        type: resource[:type],
        tags: resource[:tags],
        dataset_type: resource[:properties][:type],
        dataset_folder_path: resource[:properties][:typeProperties][:folderPath][:type],
        dataset_folder_type: resource[:properties][:typeProperties][:folderPath][:value],
        dataset_file_name: resource[:properties][:typeProperties][:fileName][:value],
        dataset_file_type: resource[:properties][:typeProperties][:fileName][:type],
        dataset_format: resource[:properties][:format],
      }
    end
  end
end
