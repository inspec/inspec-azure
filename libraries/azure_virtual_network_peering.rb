...
class AzurermVirtualNetworkPeering < AzureVirtualNetworkPeering
  name 'azurerm_virtual_network_peering'
  desc 'Verifies settings for an Azure Virtual Network Peering'
  example <<-EXAMPLE
    describe azurerm_virtual_network_peering(resource_group: 'example',vnet: 'virtual-network-name' name: 'virtual-network-peering-name') do
      it { should exist }
      its('name') { should eq 'virtual-network-peering-name' }
    end
  EXAMPLE
  ...
  def initialize(opts = {})
    Inspec::Log.warn Helpers.resource_deprecation_message(@__resource_name__, AzureVirtualNetworkPeering.name)
    # Options should be Hash type. Otherwise Ruby will raise an error when we try to access the keys.
    raise ArgumentError, 'Parameters must be provided in an Hash object.' unless opts.is_a?(Hash)
