...
class AzurePowerBIDatasetDatasources < AzureGenericResources
  ...
  def initialize(opts = {})
    ...
    Validators.validate_parameters(resource_name: @__resource_name__, allow: %i(group_id), required: %i(dataset_id),
                                   opts: opts)
    ...
    opts[:resource_uri] = ['https://api.powerbi.com/v1.0/myorg'].tap do |array|
      ...
      array << ['datasets', opts.delete(:dataset_id), 'datasources']
    end.join('/')
  ...
  def to_s
    super(AzurePowerBIDatasetDatasources)
  end
end
