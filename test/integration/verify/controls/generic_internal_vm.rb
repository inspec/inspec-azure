...
control 'azure-generic-vm-linux-internal-2.0' do
  ...
  describe azure_generic_resource(group_name: 'Inspec-Azure',
    ...
    its('properties.storageProfile.imageReference.properties.osProfile.computerName') { should eq 'linux-internal-1' }
    its('properties.storageProfile.imageReference.properties.osProfile.adminUsername') { should eq 'azure' }
    its('properties.storageProfile.imageReference.properties.osProfile.linuxConfiguration.disablePasswordAuthentication') { should be false }
