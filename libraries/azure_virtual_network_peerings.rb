...
class AzurermVirtualNetworkPeerings < AzureVirtualNetworkPeerings
  name 'azurerm_virtual_network_peerings'
  desc 'Verifies settings for Azure Virtual Network Peerings'
  example <<-EXAMPLE
    azure_virtual_network_peerings(resource_group: 'example', vnet: 'virtual-network-name') do
      it{ should exist }
  ...
  def initialize(opts = {})
    Inspec::Log.warn Helpers.resource_deprecation_message(@__resource_name__, AzurermVirtualNetworkPeerings.name)
    # Options should be Hash type. Otherwise Ruby will raise an error when we try to access the keys.
    raise ArgumentError, 'Parameters must be provided in an Hash object.' unless opts.is_a?(Hash)
