...
require 'azure_bastion_hosts_resources_test'
...
class AzureBastionHostsResourcesConstructorTest < Minitest::Test
  ...
  def tag_value_not_ok
    assert_raises(ArgumentError) { AzureBastionHostsResources.new(tag_value: 'some_tag_value') }
  end

  def tag_name_not_ok
    assert_raises(ArgumentError) { AzureBastionHostsResources.new(tag_name: 'some_tag_name') }
  end
